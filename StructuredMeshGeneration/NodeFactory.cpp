...
namespace StructuredMeshGenerator{
    ...
    void NodeFactory::Create1DBoundaryNodes(unsigned position, PhysicalSpaceEntity &space) const {
        ...
        nodesMatrix->populateElement(0, AllocateBoundaryNode(0, space));
        nodesMatrix->populateElement(position - 1, AllocateBoundaryNode(position - 1, space));
    }
    ...
    void NodeFactory::Create2DBoundaryNodes(unsigned index1, unsigned index2, PhysicalSpaceEntity &space) const {
        ...
        for (auto i = 0; i < index1 ; ++i) {
            nodesMatrix->populateElement(i, 0,  AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //Right boundary nodes.
        for (auto i = 1; i < index2 ; ++i) {
            nodesMatrix->populateElement(index1 - 1, i, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //Top boundary nodes.
        for (int i = index1 - 2; i >= 0 ; --i) {
            nodesMatrix->populateElement(i, index2 - 1, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //Left boundary nodes.
        for (auto i = index2 - 2; i >= 1 ; --i) {
            nodesMatrix->populateElement(0, i, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }         
    }
    ...
    void NodeFactory::Create3DBoundaryNodes(PhysicalSpaceEntity &space) const {
        ...
        for (int i = 0; i < _nn1 ; ++i) {
            nodesMatrix->populateElement(i, 0, 0, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //FrontRight boundary nodes.
        for (int i = 1; i < _nn2 ; ++i) {
            nodesMatrix->populateElement(_nn1 - 1, i, 0, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //FrontTop boundary nodes.
        for (int i = _nn1 - 2; i >= 0 ; --i) {
            nodesMatrix->populateElement(i, _nn2 - 1, 0, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //FrontLeft boundary nodes.
        for (auto i = _nn2 - 2; i >= 1 ; --i) {
            nodesMatrix->populateElement(0, i, 0, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //BackBottom boundary nodes.
        for (auto i = 0; i < _nn1 ; ++i) {
            nodesMatrix->populateElement(i, 0, _nn3 - 1, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //BackRight boundary nodes.
        for (int i = 1; i < _nn2 ; ++i) {
            nodesMatrix->populateElement(_nn1 - 1, i, _nn3 - 1, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //BackTop boundary nodes.
        for (int i = _nn1 - 2; i >= 0 ; --i) {
            nodesMatrix->populateElement(i, _nn2 - 1, _nn3 - 1, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //BackLeft boundary nodes.
        for (auto i = _nn2 - 2; i >= 1 ; --i) {
            nodesMatrix->populateElement(0, i, _nn3 - 1, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //BottomLeft boundary nodes.
        for (auto i = 1; i < _nn3 - 1 ; ++i) {
            nodesMatrix->populateElement(0, 0, i, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //BottomRight boundary nodes.
        for (auto i = 1; i < _nn3 - 1 ; ++i) {
            nodesMatrix->populateElement(_nn1 - 1, 0, i, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //TopLeft boundary nodes.
        for (auto i = 1; i < _nn3 - 1 ; ++i) {
            nodesMatrix->populateElement(0, _nn2 - 1, i, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
        //TopRight boundary nodes.
        for (auto i = 1; i < _nn3 - 1 ; ++i) {
            nodesMatrix->populateElement(_nn1 - 1, _nn2 - 1, i, AllocateBoundaryNode(boundaryId, space));
            boundaryId++;
        }
    }
    ...
    void NodeFactory::Create1DInternalNodes(unsigned maxIndex, PhysicalSpaceEntity &space) const {
        ...
        for (int i = 1; i < maxIndex - 1; i++){
            nodesMatrix->populateElement(i, AllocateInternalNode(internalId, space));
            internalId++;
        }
    }
    ...
    void NodeFactory::Create2DInternalNodes(unsigned index1, unsigned index2, PhysicalSpaceEntity &space) const {
        ...
        for (int i = 1; i < index1 - 1; i++){
            for (int j = 1; j < index2 - 1; j++){
                nodesMatrix->populateElement(j, i, AllocateInternalNode(internalId, space));
        ...
        for (int i = 1; i < _nn1 - 1; i++){
            for (int j = 1; j < _nn2 - 1; j++){
                for (int k = 1; k < _nn3 - 1; k++){
                    nodesMatrix->populateElement(k, j, i, AllocateInternalNode(internalId, space));
                    internalId++;
                }
            }
        }
